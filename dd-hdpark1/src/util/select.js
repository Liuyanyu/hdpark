...
$.fn.selectDate1 = function(){
	...
	var daySel1 = document.getElementById('days1')

	for(var y = maxYear;y >= minYear;y--){
		var yearOpt = document.createElement('option')
		yearOpt.value = y
		yearOpt.innerHTML = y+'年'
		yearSel1.appendChild(yearOpt)
	}

	$("#year").click(function(event){
		if(!$("#year option:selected").val()) return;
		removeOption(monthSel1)
		addOption(12,'月',monthSel1)
		removeOption(daySel1)
	})
	...
	$("#month").click(function(){
		...
		var month = $("#month option:selected").val()
		if(month==1 || month==3 || month==5 || month==7 || month==8 || month==10 || month==12){
			addOption(31,'日',daySel1)
		}else if(month==4 || month==6 || month==9 || month==11){
			addOption(30,'日',daySel1)
		}else if(month==2){
			if((year%4 == 0 && year%100 != 0 ) || (year%400 == 0)){
				addOption(29,'日',daySel1)
			}else{
				addOption(28,'日',daySel1)
			}
		}
	})
